...
namespace IMS2.BusinessModel.SatisticsValueModel
{
    public class SatisticsValue : ISatisticsValue
    {
        ...
        private Guid GetDurationIDFromTest1(Guid durationId, DateTime time, out DateTime[] times)
        {
            times = new DateTime[3];
            var duration = this.DurationList.FirstOrDefault(a => a.DurationId == durationId);
            if (duration != null)
            {
                ...
                return this.DurationList.Find(a => a.Level == duration.NextLevel()).DurationId;
            }
            else
            {
                return Guid.Empty;
